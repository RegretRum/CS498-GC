...
class OdometryNode(Node):
    ...
    def __init__(self):
        ...
        self.subscription_Blspeed = self.create_subscription(Float32,'Blspeed',self.callback_Bls,10)
        self.subscription_Brspeed = self.create_subscription(Float32,'Brspeed',self.callback_Brs,10)
        self.subscription_Flspeed = self.create_subscription(Float32,'Flspeed',self.callback_Fls,10)
        self.subscription_Frspeed = self.create_subscription(Float32,'Frspeed',self.callback_Frs,10)
        self.subscription_latitude = self.create_subscription(Log,'latitude',self.callback_la,10)
        self.subscription_longitude = self.create_subscription(Log,'longitude',self.callback_lo,10)
    ...
    def timer_callback_odom(self):
        ...
        w = self.gyro_yaw # ... Angular velocity of the robot
        self.x = self.x + v * math.cos(self.theta) * dt # ...Position
        self.y = self.y + v * math.sin(self.theta) * dt # ...Position
        self.theta = self.theta + w*dt # ...Heading angle
